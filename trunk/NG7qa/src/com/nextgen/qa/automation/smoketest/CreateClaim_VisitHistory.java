...
public class CreateClaim_VisitHistory extends NG7TestCase {
	  
//private static WebElement temp;
//private static BufferedWriter document;

@Test
public static void test() throws Exception {
	NG7TestCase.testName = "CreateClaim_VisitHistory";	
	...
	for (int i = 1; i <= localStressLoop; i++){
		...
		driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
					...
					currStepResult = (preReq_openPatientCenter.equals("Pass") == false) ? "Blocked" :
						...
						pdb.checkElementIsVisible(pdb.findVisibleElement(pdb.openPatientDashboard, pdb.HeaderBlockLinksRightButtons), "Claims link") ? "Pass" : "Fail";
					//pdb.clickButton("Check Eligibility", "Insurance Tab Check Eligibility") ? "Pass" : "Fail";
					//pdb.clickButton1("CreateClaims", "Visit History create claim") ? "Pass" : "Fail";
					GeneralMethods.clickButton1(pdb.findVisibleElement(pdb.openPatientDashboard, pdb.HeaderBlockLinksRightButtons) "Claims link") ? "Pass" : "Fail"; 
					Artifact.VerifyWriteToArtifactS(artifact, iterationStamp+"Visit History create claim", currStepResult);
					prevStepResult = currStepResult;				
										
					/*public String CheckEligibility;
					currStepResult = preReq_patientTabOpened.equals("Pass") == false ? "Blocked" :
						GeneralMethods.ClickButton(pdb.getHeaderBlockLinksRightButtons, buttonDescription) Create Claim) ? "Pass" : "Fail";
					currStepResult= pdb.clickButton1("Create Claim", "Visit History Create claim") ? "Pass" : "Fail";
						pdb.clickButton1(HeaderBlockLinksRightButtons, "Visit History Create claim") ? "Pass" : "Fail";
				...
				prevStepResult = currStepResult;
					...
					//public String CheckEligibility;
						//currStepResult = preReq_patientTabOpened.equals("Pass") == false ? "Blocked" :
							GeneralMethods.ClickButton(pdb.getHeaderBlockLinksRightButtons(buttonCssSelector, buttonCssSelector), buttonDescription)Create Claim) ? "Pass" : "Fail";
						//currStepResult= pdb.clickTab("Insurance", "Patient Dashboard Insurance Tab") ? "Pass" : "Fail";
							pdb.clickButton1(buttonCssSelector, HeaderBlockLinksRightButtons)
							GeneralMethods.ClickButton(pdb.HeaderBlockLinksRightButtons pdb.openPatientDashboard, pdb.linkChargesCss)) ? "Pass" : "Fail";
						//Artifact.VerifyWriteToArtifactS(artifact, iterationStamp+"Click checkEligibilityInsurance", currStepResult);
					//	prevStepResult = currStepResult;
						//String preReq_financialGridOpened = currStepResult;*/
	...
	//	temp = pdb.findVisibleElement(pdb.financialGridRight, pdb.linkClaimsCss);
	//System.out.println();
	
	//var temp = document.getElementsByClassName("patient-dash-header-info-box");
	//temp[0].getFirstChild().getChildNotes(1);
	//if (a.innerHTML == "+ Claims") {... };
		...
		//if (temp != null)
				...
				/*currStepResult = (preReq_openPatientCenter.equals("Pass") == false) ? "Blocked" :
					//pdb.clickButton("Check Eligibility", "Insurance Tab Check Eligibility") ? "Pass" : "Fail";
					GeneralMethods.ClickButton(temp.div[0].a[0]) ? "Pass" : "Fail"; // MGB 3/9/2015 updated to use css
				Artifact.VerifyWriteToArtifactS(artifact, iterationStamp+"Create Claim Button", currStepResult);
				prevStepResult = currStepResult;
				//String preReq_financialGridOpened = currStepResult;
				*/}
