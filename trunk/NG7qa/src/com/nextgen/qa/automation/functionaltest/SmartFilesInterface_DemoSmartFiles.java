	...
	public void test() throws Exception {
		...
		for (int i = 1; i <= localStressLoop; i++){ // localStressLoop is controlled via the config.properties file
			...
			for (i=0; i < smartFileType.length; i++){
				...
				// Skip any blocked smarttypes
				if (smartFileType[i].equals("Activity")) {
					Artifact.VerifyWriteToArtifactS(artifact, iterationStamp+"Activity smart file requirement not completely implemented yet", "Blocked"+knownIssue+"NGTRUNK-17636");
					continue;
				}
				else if (smartFileType[i].equals("GH Provider")) {
					Artifact.VerifyWriteToArtifactS(artifact, iterationStamp+"Provider not saving properly", "Blocked"+knownIssue+"NGTRUNK-19100");
					continue;
				}
